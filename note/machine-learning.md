...
# 2.构建KNN算法，实例化KNN算法 使用KNN训练
# 2.1第一步实例化KNN算法
n_neighbors=5 #确定K等于几 这里是5
#
knn=KNeighborsClassifier(n_neighbors=n_neighbors,algorithm='brute',p=2) #s实例化k近邻算法
# 训练
knn.fit(point_concat,point_concat_label)
# 3.实现KNN决策边界
#获得预测点数据
#3.1 设定未知点 ，使用坐标点网格来当作预测数据点
x1=np.linspace(0,10,100) #第一个参数是数据开始点 第二个参数为数据结束点 第三个参数选取的个数
y1=np.linspace(0,10,100) #第一个参数是数据开始点 第二个参数为数据结束点 第三个参数选取的个数
#生成坐标点网格
x_axis,y_axis=np.meshgrid(x1,y1) #生成坐标点网格，返回的2个2维矩阵可以组成，预测数据点矩阵(2维)
y_axis.shape
#将x_axis,y_axis进行展平 用x_axis.ravel() 或则用x_axis.flatten()
x_axis_ravel=x_axis.ravel()
y_axis_ravel=y_axis.ravel()
#可以用np.concatenate()函数合并，也可以用np.c_默认按照列上进行合并
...
#contour表示画等高线，第一个参数表示二维的坐标点网格，第二个表示二维的坐标点网格，(第一个参数和第二个参数
# 组合起来就是模型需要输入的二维坐标点数据)，第三个参数是不是就是输入二维坐标点的预测值。
ax.contour(x_axis,y_axis,knn_predict_result.reshape(x_axis.shape))
#绘制原始点
...
ax.scatter(point_concat[point_concat_label==2,0],point_concat[point_concat_label==2,1],color='y',marker='s')
plt.show()
